...
#define SAMPLE_CHECK_ERRORS(ERR)
	...
	OvrvisionPro::OvrvisionPro(int width, int height)
	{
		...
		mapX[1] = new Mat();
		mapY[1] = new Mat();
		_l = clCreateImage2D(_context, CL_MEM_READ_WRITE, &_format8UC4, size.width, size.height, 0, 0, &_errorCode);
		_r = clCreateImage2D(_context, CL_MEM_READ_WRITE, &_format8UC4, size.width, size.height, 0, 0, &_errorCode);
		_L = clCreateImage2D(_context, CL_MEM_READ_WRITE, &_format8UC4, size.width, size.height, 0, 0, &_errorCode);
		_R = clCreateImage2D(_context, CL_MEM_READ_WRITE, &_format8UC4, size.width, size.height, 0, 0, &_errorCode);
		_mx[0] = clCreateImage2D(_context, CL_MEM_READ_ONLY, &_formatMap, size.width, size.height, 0, 0, &_errorCode);
	...
	bool OvrvisionPro::LoadCameraParams(const char *filename)
	{
		...
		{
			// Left camera
			_settings.GetUndistortionMatrix(OV_CAMEYE_LEFT, *mapX[0], *mapY[0], size.width, size.height);
			_errorCode = clEnqueueWriteImage(_commandQueue, _mx[0], CL_TRUE, origin, region, size.width * sizeof(float), 0, mapX[0]->ptr(0), 0, NULL, NULL);
			_errorCode = clEnqueueWriteImage(_commandQueue, _my[0], CL_TRUE, origin, region, size.width * sizeof(float), 0, mapY[0]->ptr(0), 0, NULL, NULL);
			SAMPLE_CHECK_ERRORS(_errorCode);

			// Right camera
			_settings.GetUndistortionMatrix(OV_CAMEYE_RIGHT, *mapX[1], *mapY[1], size.width, size.height);
			_errorCode = clEnqueueWriteImage(_commandQueue, _mx[1], CL_TRUE, origin, region, size.width * sizeof(float), 0, mapX[1]->ptr(0), 0, NULL, NULL);
			_errorCode = clEnqueueWriteImage(_commandQueue, _my[1], CL_TRUE, origin, region, size.width * sizeof(float), 0, mapY[1]->ptr(0), 0, NULL, NULL);
			SAMPLE_CHECK_ERRORS(_errorCode);
	...
	void OvrvisionPro::Demosaic(const Mat src, Mat &left, Mat &right)
	{
		...
		_errorCode = clEnqueueWriteImage(_commandQueue, _src, CL_TRUE, origin, region, src.cols * sizeof(ushort), 0, ptr, 0, NULL, &writeEvent);
		SAMPLE_CHECK_ERRORS(_errorCode);
	...
	void OvrvisionPro::DemosaicRemap(const Mat src, Mat &left, Mat &right)
	{
		...
		if (_remapAvailable)
		{
			size_t remapSize[] = { src.cols, src.rows };
