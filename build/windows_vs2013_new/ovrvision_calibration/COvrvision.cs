...
namespace ovrvision_calibration
{
	public class COvrvision
	{
		...
		//Open Ovrvision
		public bool Open(int hmd)
		{
			if (camStatus)
				return false;

			//Open camera
			if (ovOpen(0, 0.15f, hmd) == 0)
			{
				camStatus = true;
				g_hmd = hmd;
			} else {
		...
		public void UpdateLeft()
		{
			...
			if(g_hmd==VERSION2_DK2)
				imageDataLeft.RotateFlip(RotateFlipType.Rotate180FlipNone);
		}
		...
		public void UpdateRight()
		{
			...
			if (g_hmd == VERSION2_DK2)
				imageDataRight.RotateFlip(RotateFlipType.Rotate180FlipNone);
		}
